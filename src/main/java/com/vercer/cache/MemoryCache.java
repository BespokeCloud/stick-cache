...
public class MemoryCache<K, V> extends Cache.Accessor implements Cache<K, V>
{
	...
	public MemoryCache(int capacity, boolean expire, boolean intern)
	{
		...
		this.items = new LinkedHashMap<K, CacheItem<K, V>>(capacity, 0.75f, true)
		{
			...
			protected boolean removeEldestEntry(Map.Entry<K, CacheItem<K, V>> eldest)
			{
				...
				if (MemoryCache.this.expire && operations++ >= check)
				{
	...
	{
		lock.readLock().lock();
		CacheItem<K, V> item = items.get(key);
		accessed(item);
	...
	{
		HashMap<K, CacheItem<K, V>> result = new HashMap<K, CacheItem<K, V>>(keys.size());
		for (K key : keys)
		{
			CacheItem<K, V> item = item(key);
			if (item != null)
